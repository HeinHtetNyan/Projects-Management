...
class Secret(Base):
    ...
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=True)
    environment = Column(String(50), nullable=True)   # development, staging, production, global
...
class SecretVersion(Base):
    ...
    encrypted_value = Column(Text, nullable=False)   # Fernet-encrypted snapshot
...
class Domain(Base):
    ...
    status = Column(String(20), nullable=False, default="active")   # active, expired, transferred
    notes = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class Integration(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    service = Column(String(100), nullable=False)     # Cloudflare, GitHub, Vercel, OpenAI, etc.
    account = Column(String(255), nullable=True)
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=True)
    related_secrets = Column(Text, nullable=True)     # comma-separated secret names
    notes = Column(Text, nullable=True)
    status = Column(String(20), nullable=False, default="active")
    created_at = Column(DateTime, default=datetime.utcnow)
